...
const renderAssignmentTable = async (eventData) => {
  ...
  const { problems, problemAssignments } = eventData;
  ...
  const isChecked = (problemId, categoryId, problemAssignments) => {
    const entry = problemAssignments.find((assignment) => {
      return assignment.problemId === problemId && assignment.categoryId === categoryId;
    });
  ...
  const createRow = (problem, categoryIds, problemAssignments ) => {
    ...
    categoryIds.forEach((categoryId) => {
      const checked = isChecked(problem.id, categoryId, problemAssignments);
  ...
  problems.map((problem) => {
    const rowEl = createRow(problem, categoryIds, problemAssignments);
